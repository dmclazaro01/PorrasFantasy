-- ============================================================
-- Porra Fantasy · 0034 · Excepción CANCHERO solo para la J6
-- ============================================================
-- La ventana ampliada del CANCHERO (jugable hasta el comienzo del
-- segundo partido, 0033) queda limitada a la jornada 11 (Jornada 6,
-- que arrancó antes de existir la app). El resto de jornadas vuelven
-- a la regla estricta: solo antes de empezar. Si la fila de rounds se
-- reconstruyera con otro id, la excepción deja de aplicar sola (vuelta
-- segura a la regla estricta).
-- ============================================================
...
declare
  c porra.cards; m porra.matches; my_points int; started_count int; played_count int;
begin
