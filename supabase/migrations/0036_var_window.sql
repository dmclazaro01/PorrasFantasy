-- ============================================================
-- Porra Fantasy · 0036 · Ventana del VAR (24 h) + cross-round
-- ============================================================
-- El VAR se puede jugar hasta 24 h después del comienzo del ÚLTIMO
-- partido finalizado DE SU JORNADA (la del partido objetivo, no la de
-- la carta). Por eso la carta ya no está atada a su jornada: vale
-- cualquier GRANTED VAR propio sobre un FINISHED de la misma
-- competición de la sala en plazo (una sala sin competición no
-- restringe). Sin este cambio, en cuanto la app avanza de jornada los
-- finalizados anteriores quedaban fuera de alcance aunque la carta
-- siguiera sin usar.
-- Se mantiene: un solo gol, objetivo propio o un rival, veto sobre su
-- FINALISSIMA, unicidad por (sala, partido, afectado) y recálculo
-- inmediato. El cálculo (propio primero, luego el más antiguo) ya
-- soportaba varios VAR por partido y no cambia.
-- ============================================================
...
declare
  c porra.cards; m porra.matches; my_points int; started_count int; played_count int; last_fin timestamptz;
begin
